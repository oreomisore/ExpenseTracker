...
# 2. Connecting to database and creating the gUI window:
db = sqlite3.connect("ExpenseTracker.db")
cursor = db.cursor()

db.execute('CREATE TABLE IF NOT EXISTS ExpenseTracker '
...
def view_expense_details():
    ...
    if not table.selection():
        mb.showerror('No expense selected', 'Please select expense from the table to view its details')

    current_selected_expense = table.item(table.focus())
    values = current_selected_expense['values']

    expenditure_date = datetime.date(int(values[1][:4]),
...
def remove_expense():
    ...
    if surety:
        db.execute('DELETE FROM ExpenseTracker WHERE ID=%d' % values_selected[0])
        db.commit()
...
def selected_expense_to_words():
    ...
    message = f'Your expense can be read like: \n"You paid {values[4]} to {values[2]} for {values[3]} on {values[1]} ' \
              f'via {values[5]}"'
...
def expense_to_words_before_adding():
    ...
    message = f'Your expense can be read like: \n"You paid {amnt.get()} to {payee.get()} for {desc.get()} on ' \
              f'{date.get_date()} via {MoP.get()}" '
...
# Window backgrounds and fonts
dataentery_frame_bg = 'Slategray1'
buttons_frame_bg = 'ivory2'
hlb_btn_bg = 'ivory2'
...
button_frame = Frame(root, bg=buttons_frame_bg)
...
# Data Entry Frame
Label(data_entry_frame, text='Data (M/DD/YY) :',
      font=lbl_font, bg=dataentery_frame_bg).place(x=10, y=5)
date = DateEntry(data_entry_frame, date=datetime.datetime.now().date(), font=entry_font)
date.place(x=160, y=50)

Label(data_entry_frame, text='Payee\t             :',
      font=lbl_font, bg=dataentery_frame_bg).place(x=10, y=230)
Entry(data_entry_frame, font=entry_font, width=31, text=payee).place(x=10, y=260)

Label(data_entry_frame, text='Description           :',
      font=lbl_font, bg=dataentery_frame_bg).place(x=10, y=100)
Entry(data_entry_frame, font=entry_font, width=31, text=desc).place(x=10, y=130)

Label(data_entry_frame, text='Amount\t             :',
      font=lbl_font, bg=dataentery_frame_bg).place(x=10, y=180)
Entry(data_entry_frame, font=entry_font, width=14, text=amnt).place(x=160, y=180)

Label(data_entry_frame, text='Mode of Payment: ',
      font=lbl_font, bg=dataentery_frame_bg).place(x=10, y=310)
...
Button(data_entry_frame, text='Convert to words before adding', font=btn_font, width=30, bg=hlb_btn_bg).place(x=10,
                                                                                                              y=450)

# Buttons' Frame
Button(button_frame, text='Delete Expense', font=btn_font, width=25, bg=hlb_btn_bg, command=remove_expense).place(x=30,
                                                                                                                  y=5)
Button(button_frame, text='Clear Fields in DataEntry Frame', font=btn_font, width=25, bg=hlb_btn_bg,
       command=clear_fields).place(x=335, y=5)
Button(button_frame, text='Delete All Expenses', font=btn_font, width=25, bg=hlb_btn_bg,
       command=remove_all_expenses).place(x=640, y=5)
Button(button_frame, text='View Selected Expense\'s Details', font=btn_font, width=25, bg=hlb_btn_bg,
       command=view_expense_details).place(x=30, y=65)
Button(button_frame, text='Edit Selected Expense', command=edit_expense, font=btn_font, width=25, bg=hlb_btn_bg).place(
    x=335, y=65)
Button(button_frame, text='Convert Expense to a sentence', font=btn_font, width=25, bg=hlb_btn_bg,
